...
namespace NodeCanvas.Tasks.Actions{
	...
	[Category("Level")]
	public class LoadLevelIndex : ActionTask{
        [RequiredField]
        public BBParameter<int> levelIndex;
...
#else
                asyncOperation = SceneManager.LoadSceneAsync(levelIndex.value, mode.value);
#endif
                Debug.Log(string.Format("LoadLevelAsyc{0}:{1} ", mode.value.ToString(), levelIndex.value));
            }
            else
            {
#if UNITY_PRE_5_3
...
#else
                SceneManager.LoadScene(levelIndex.value, mode.value);
#endif

                Debug.Log(string.Format("LoadLevel{0}:{1} " , mode.value.ToString(), levelIndex.value));
                progress.value = 1f;
        ...
        private AsyncOperation LoadSceneAsync(int name, LoadSceneMode mode)
        {
            switch (mode)
            {
                case LoadSceneMode.Single:
        ...
        private void LoadScene(int name, LoadSceneMode mode)
        {
