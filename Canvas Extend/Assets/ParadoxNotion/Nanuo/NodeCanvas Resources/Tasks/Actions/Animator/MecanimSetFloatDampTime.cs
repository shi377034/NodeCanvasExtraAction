...
namespace NodeCanvas.Tasks.Actions{
	...
	public class MecanimSetFloatDampTime : ActionTask<Animator> {
		...
		void Set(float newValue){
			if (!string.IsNullOrEmpty(parameter.value)){
				agent.SetFloat(parameter.value, newValue,dampTime.value,Time.deltaTime);
				return;
			}
			agent.SetFloat(parameterHashID.value, newValue,dampTime.value,Time.deltaTime);
		}
	}
}